...
<script>
	...
	export default {
		...
		setup() {
			...
			const getReceiveRank = () => {
				state.loading = true
				axios.get('/api/back/receiveRanks', {
					params: {
			...
			const handleExport = () => {
				axios.get('/api/back/receiveRanks/export', {
					params: {
						office_name: state.office_name,
						date: state.date,
					},
					...
					let end_date = new Date(state.date[1]).getFullYear() + '/' + (new Date(state.date[1])
						.getMonth() + 1)
					link.setAttribute('download', state.office_name + '-接单排名-' + start_date + '-' + end_date +
						'.xlsx');
